...
import menuContentModule from 'dan-api/ui/menu';

// Dinamik menu için
const getMenuContent = () => {
  if (typeof menuContentModule.getMenu === 'function') {
    return menuContentModule.getMenu();
  }
  return menuContentModule;
};
  ...
  for (let i = 0; i < arr.length; i += 1) {
    for (let j = 0; j < arr[i].length; j += 1) {
      if (arr[i][j].link === curRoute) {
        headMenu = currentMenuContent[i].key;
      }
    }
  }
  return headMenu;
