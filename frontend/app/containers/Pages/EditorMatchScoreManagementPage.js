...
import { authAPI, editorAPI, matchesAPI } from 'utils/api';

const authService = new AuthService(authAPI);

/**
 * Editor Maç Skor Yönetimi Sayfası (Command Pattern)
 * 
 * Bu sayfa Command Pattern kullanarak:
 * - Kendi eklediği maçlara skor girişi yapar
 * - Maçları sonlandırır
 * - İşlemleri geri alabilir (UNDO) ⭐
 * - İşlem geçmişini gösterir
 */
function EditorMatchScoreManagementPage() {
  ...
  /**
   * Editörün maçlarını yükle
   */
  const loadEditorMatches = async () => {
    try {
      setLoading(true);
      const data = await editorAPI.getMyMatches();
      console.log('📊 Editor - Gelen maçlar:', data);
      console.log('📊 Maç sayısı:', data?.length);
      ...
      const approvedMatches = Array.isArray(data) 
        ? data.filter(m => {
      ...
      console.log('✅ Filtrelenmiş maçlar:', approvedMatches);
  ...
  const loadCommandHistory = async () => {
    try {
      const response = await editorAPI.getCommandHistory();
  ...
  const handleScoreSubmit = async () => {
    ...
    try {
      ...
      const result = await editorAPI.updateMatchScoreCommand(payload);
  ...
  const handleFinishMatch = async () => {
    ...
    try {
      ...
      const result = await editorAPI.finishMatchCommand(payload);
  ...
  const handleUndo = async () => {
    ...
    try {
      ...
      const result = await editorAPI.undoLastCommand();
  ...
  return (
    <Container maxWidth="lg" sx={{ mt: 4, mb: 4 }}>
      <PapperBlock
        ...
        icon="ios-football"
        desc="Maçlara skor girin, sonlandırın ve işlemleri geri alın! ⚡"
      >
        {/* Üst Butonlar */}
        <Box mb={3}>
          <Stack direction="row" spacing={2}>
            ...
            <Button
              variant="outlined"
              color="primary"
              onClick={() => navigate('/app/editor/matches/add')}
            >
              Yeni Maç Ekle
            </Button>
          </Stack>
            ...
            <Grid item xs={12}>
              <Paper sx={{ p: 3, textAlign: 'center' }}>
                <Typography color="text.secondary">
                  Henüz onaylanmış maçınız bulunmuyor
                </Typography>
                <Button
                  variant="contained"
                  color="primary"
...
export default EditorMatchScoreManagementPage;
