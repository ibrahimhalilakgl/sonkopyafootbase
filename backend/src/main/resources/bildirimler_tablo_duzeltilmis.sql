-- Bildirimler Tablosu Oluşturma Script'i (Düzeltilmiş)
...
CREATE TABLE IF NOT EXISTS bildirimler (
    ...
    -- İlgili oyuncu (Opsiyonel) - oyuncu tablosunda 'id' kolonu kullanılıyor
    oyuncu_id BIGINT,
    CONSTRAINT fk_bildirim_oyuncu FOREIGN KEY (oyuncu_id) 
        REFERENCES oyuncular(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_bildirim_alici ON bildirimler(alici_kullanici_id);
CREATE INDEX IF NOT EXISTS idx_bildirim_okundu ON bildirimler(alici_kullanici_id, okundu);
CREATE INDEX IF NOT EXISTS idx_bildirim_tarih ON bildirimler(olusturma_zamani DESC);
CREATE INDEX IF NOT EXISTS idx_bildirim_tip ON bildirimler(bildirim_tipi);
